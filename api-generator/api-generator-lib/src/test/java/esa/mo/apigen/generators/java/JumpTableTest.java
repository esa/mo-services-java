...
/**
 * Tests the rules that decide how far the switch of an area factory reaches
 * before the rest of the type numbers are moved out to a second method. Carried over
 * from the generator this replaces, so that the two lay their cases out alike.
 */
public class JumpTableTest {
    ...
    public void bandCoversAnAreaThatHasNoGaps() {
        assertEquals(12, JumpTable.widestJumpTableBand(withLists(range(1, 12))));
        assertEquals(45, JumpTable.widestJumpTableBand(withLists(range(1, 45))));
    }
    ...
    public void bandReachesPastAGapThatTheNumbersBehindItPayFor() {
        ...
        assertEquals(60, JumpTable.widestJumpTableBand(numbers));
    }
    ...
    public void bandIsAnchoredAtOneAndNotOnTheLongestRun() {
        ...
        int band = JumpTable.widestJumpTableBand(numbers);
    ...
    public void aSingleTypeHasNoBand() {
        assertEquals(0, JumpTable.widestJumpTableBand(withLists(1)));
    }
    ...
    public void aLoneNumberFarOutIsLeftOutOfTheBand() {
        ...
        assertEquals(20, JumpTable.widestJumpTableBand(numbers));
    }
    ...
    public void chosenBandIsTheWidestThatStillCompilesToAJumpTable() {
        ...
        assertTrue("The chosen band is not a jump table",
                JumpTable.compilesToJumpTable(inside, 2L * band + 1L));
        assertTrue("The whole area would still be a jump table",
                !JumpTable.compilesToJumpTable(numbers.size(), 2L * 1010 + 1L));
    }
    ...
    public void noTypesGiveNoBand() {
        assertEquals(0, JumpTable.widestJumpTableBand(new ArrayList<Integer>()));
    }
    ...
    public void numbersFarFromZeroAreSwitchedOneSideAtATime() {
        assertTrue(JumpTable.shouldSplitOnSign(withLists(range(1001, 1010))));
    }
    ...
    public void numbersThatAreAlreadyAJumpTableAreLeftWhole() {
        assertFalse(JumpTable.shouldSplitOnSign(withLists(range(1, 19))));
        assertFalse(JumpTable.shouldSplitOnSign(withLists(range(1, 105))));
    }
    ...
    public void aSingleTypeFarFromZeroIsLeftWhole() {
        assertFalse(JumpTable.shouldSplitOnSign(withLists(90)));
    }
    ...
    public void numbersOnOneSideOfZeroAreLeftWhole() {
        ...
        assertFalse(JumpTable.shouldSplitOnSign(positives));
        assertFalse(JumpTable.shouldSplitOnSign(new ArrayList<Integer>()));
    }
    ...
    public void bothSidesHaveToBeWorthAJumpTable() {
        ...
        List<Integer> lopsided = new ArrayList<Integer>();
        ...
        assertFalse(JumpTable.shouldSplitOnSign(lopsided));
    }
}
