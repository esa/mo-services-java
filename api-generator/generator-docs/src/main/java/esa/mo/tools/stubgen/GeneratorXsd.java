...
public class GeneratorXsd extends GeneratorDocument {
    ...
    @Override
    public CompositeField createCompositeElementsDetails(TargetWriter file, boolean checkType, String fieldName, TypeReference elementType, boolean isStructure, boolean canBeNull, String comment) {
        CompositeField ele;
    ...
    private class XsdWriter extends AbstractWriter {
        ...
        protected XsdWriter(String folder, String fileName, String areaName, String ext) throws IOException {
            ...
            file.append(makeLine(0, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>", false));
            file.append(makeLine(0, "<xsd:schema xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"", false));
            file.append(makeLine(0, "            targetNamespace=\"http://www.ccsds.org/schema/" + areaName + "Types\"", false));
        ...
        public void addXsdStatement(int tabCount, String statement) {
            buffer.append(makeLine(tabCount, statement, false));
        }
        ...
        public void flush() throws IOException {
            for (String area : areas) {
                file.append(makeLine(0, "            xmlns:" + area.toLowerCase() + "Types=\"http://www.ccsds.org/schema/" + area + "Types\"", false));
            }
            file.append(makeLine(0, "            elementFormDefault=\"qualified\">", false));

            for (String area : areas) {
                if (!area.equalsIgnoreCase(thisArea)) {
                    file.append(makeLine(1, "<xsd:import namespace=\"http://www.ccsds.org/schema/" + area + "Types\" schemaLocation=\"ServiceDef" + area + "Types.xsd\"/>", false));
                }
            }

            file.append(buffer.toString());
            file.append(makeLine(0, "</xsd:schema>", false));
            file.flush();
        }
