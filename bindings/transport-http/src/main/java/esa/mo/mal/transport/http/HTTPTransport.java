...
public class HTTPTransport extends Transport<HTTPHeaderAndBody, byte[]> {
  ...
  /**
   * Always use HTTP requests to deliver MAL messages (the HTTP response is "204 No Content" on all occasions), but do
   * not encode the MAL message header separately via HTTP headers
   */
  private static final int HTTP_BINDING_MODE_NO_ENCODING = 0;

  /**
   * Always use HTTP requests to deliver MAL messages (the HTTP response is "204 No Content" on all occasions), and
   * additionally encode the MAL message header separately via HTTP headers
   */
  private static final int HTTP_BINDING_MODE_NO_RESPONSE = 1;

  /**
   * Use HTTP request/response paradigm to map as many interaction type operations as possible for the remaining
   * operations HTTP requests go in the other direction (see Table 3-3 in recommended standard),
   */
   ...
   */
  public static final String HTTP_DEFAULT_XML_ENCODER = "esa.mo.mal.encoder.http.HTTPXMLStreamFactory";
  ...
  /**
   * Store for mappings from URI+transactionId to open HTTP response for messages where a subsequent MAL message is
   * expected after submitting the HTTP request and needs to be encoded and passed within the HTTP response
   */
  private final Map<String, IHttpResponse> openHttpResponses = Collections
      .synchronizedMap(new HashMap<String, IHttpResponse>());

  /**
   * This Executor can be used to run submitted tasks asynchronously (e.g. deferred processing the HTTP response after
   ...
   *             in case an error occurs when trying to instantiate the AbstractHttpServer
   */
   ...
   * 
   * @param messageSource
   *            The input HTTPHeaderAndBody data structure to use.
   ...
   * @param transactionId
   *            the transactionId
   * @param abstractHttpResponse
   *            the open AbstractHttpResponse
   */
   ...
   * 
   * @param task
   *            the Runnable to submit for execution
   */
  ...
  /**
   * According to Table 3-3 in recommended standard, checks whether the MAL interaction type and stage require the
   * message to be a simple HTTP request without the need for further processing.
   * 
   * @param header
   *            the MALMessageHeader
  ...
  /**
   * According to Table 3-3 in recommended standard, checks whether the MAL interaction type and stage require the
   * message to further process the HTTP response for a subsequent MAL message after submitting the HTTP request.
   * 
   * @param header
   *            the MALMessageHeader
  ...
  /**
   * According to Table 3-3 in recommended standard, checks whether the MAL interaction type and stage require the
   * message to be encoded and passed directly within a HTTP response.
   * 
   * @param header
   *            the MALMessageHeader
