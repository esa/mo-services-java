...
public class JMSConsumeHandler extends JMSQueueHandler {
    ...
    public JMSConsumeHandler(JMSEndpoint endPoint, Object interruption,
            Session qs, Topic messageSource, String sourceName, UShort area,
            UShort service, UShort operation, UOctet version) throws Exception {
    ...
    public void register(JMSTransport transport, final String providerExchangeName,
        ...
        for (SubscriptionFilter filter : filters) {
            ...
            if ((null != sdl) && (0 < sdl.size())) {
                ...
                pvalueSet = createRoutingKeyString(pbuf,
                        JMSEndpoint.DOM_PROPERTY, subdomain, wildcard, false);
            } else {
                pvalueSet = createRoutingKeyString(pbuf,
        ...
        try {
            ...
            JMSTransport.RLOGGER.log(Level.WARNING,
                    "JMS Error occurred when subscribing {0}", e);
        }
    }
    ...
    public void deregister(boolean clearTransId) throws MALException {
        ...
        for (MessageConsumer consumer : consumerList) {
            try {
                ...
                JMSTransport.RLOGGER.log(Level.WARNING,
                        "JMS Error occurred when unsubscribing {0}", e);
            }
        }
    ...
    protected static boolean createRoutingKeyIdentifier(StringBuilder buf,
            String propertyId, Identifier id) {
    ...
    protected static boolean createRoutingKeyLong(StringBuilder buf,
            String propertyId, Long i, boolean withPrevious) {
        if (null != i) {
            if (0 != i) {
                if (withPrevious) {
                    buf.append(" AND ");
    ...
    protected static boolean createRoutingKeyString(StringBuilder buf,
            String propertyId, String i, boolean substringMatch, boolean withPrevious) {
        if (null != i) {
            if (!i.contentEquals("")) {
                if (withPrevious) {
                    buf.append(" AND ");
    ...
    protected static boolean createRoutingKeyBoolean(StringBuilder buf,
            String propertyId, boolean b, boolean withPrevious) {
        if (withPrevious) {
            buf.append(" AND ");
        }
        buf.append(propertyId);
