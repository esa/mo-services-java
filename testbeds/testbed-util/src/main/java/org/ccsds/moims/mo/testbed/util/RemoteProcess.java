    ...
    public void runProcess(String[] argv) throws Exception {
        ...
        if (argv.length > 2) {
            try {
                ...
                logMessage(new FileWriter(File.createTempFile("zzz_ERROR_LOG", ".txt")),
                        "ERROR: First parameter to RemoteProcess must be a numeric log file number, received: " + argv[0]);
            }
        }
        ...
        /**
         * Should have at least three arguments, the second is the port number
         * to open and listen on for the termination signal the second is the
         * class to instatiate and execute
         */
        if (argv.length > 2) {
            Executable obj = null;
            ...
            /**
             * we execute the test outside of the above exception handlers to
             * that any exceptions thrown by the test are not hidden.
             */
            if (null != obj) {
