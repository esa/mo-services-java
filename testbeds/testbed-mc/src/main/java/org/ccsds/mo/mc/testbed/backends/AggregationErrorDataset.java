...
import org.ccsds.moims.mo.mc.backends.AggregationBackend.DefaultReportConfiguration;
...
/**
 * AggregationErrorDataset implements the dataset #AG-9.
 */
public class AggregationErrorDataset extends AggregationDataset {
	...
	public static final ObjectRef<AggregationDefinition> sat1BcMtq1RevRef =
			new ObjectRef<>(
					Constant.DOMAIN_SAT1,
					AggregationDefinition.TYPE_ID.getTypeId(),
					Constant.ID_BC_MTQ1_REV,
					new UInteger(1));
	public final int sat1BcMtq1RevAdId;

	public static final ObjectRef<AggregationDefinition> sat2BcMtq1Ref =
			new ObjectRef<>(
					Constant.DOMAIN_SAT2,
					AggregationDefinition.TYPE_ID.getTypeId(),
					Constant.ID_BC_MTQ1,
					new UInteger(1));
	public final int sat2BcMtq1AdId;

	public AggregationErrorDataset(BackendTimer timer, SingleConnectionDetails parameterConnectionDetails) {
		...
		sat1BcMtq1RevAdId =
				addAggregation(
						...
						new ObjectRefList(new ArrayList<> (Arrays.asList(
								ParameterDefaultDataset.sat1Mtq1EnabledRef,
								ParameterDefaultDataset.sat1Mtq1VoltageRef))),
						new DefaultReportConfiguration(false, 301*1000));

		// AggregationDefinition
		// - identity: ("fr.cnes.mission.sat2", "AGG_BC_MTQ1", version=1)
		// - description: ""
		// - category: null
		// - parameters: {
		// -- {domain="fr.cnes.mission.sat1", key="ATT_BC_MTQ1VOLTAGE", version=1}
		...
		// default report interval for the provider is 120s
		addDefaultReportConfig(new DefaultReportConfiguration(true, 120*1000));
	}
