...
public enum TCPIPConnectionPoolManager
{
  ...
  /**
   * The internal list of sockets. Each socket is identified by a hash, which is made purely from
   * the port number.
   */
  ...
  /**
   * Returns a socket bound to a specific port. If this socket doesn't exist, this method will try
   * once to create a socket on the same port. If that is not possible, a socket with an ephemeral
   * port number will be created and returned.
   *
   * @param localPort The port number of the socket to return. Returns a socket with an ephemeral
   *                  port number if the localPort is occupied or not existant.
  ...
  /**
   * Get a unique hash, based on a port number. Each socket is identified by a hash, which is made
   * purely from the port number. Any instance of the MAL transport TCPIP binding always only runs
   * on one machine, and on any one machine each port number is unique. Hence, the hash is unique.
   *
  ...
  /**
   * Create a socket at a predefined port. If this is not possible, because the port is occupied or
   * otherwise unavailable, this method tries to create a socket with an ephemeral port number
   * exactly once.
   *
