 ...
 * org.ccsds.moims.mo.mal.transport.tcpip.numconnections ==> number of connections to a different MAL (either server / client) 
 * org.ccsds.moims.mo.mal.transport.tcpip.inputprocessors ==> number of threads processing in parallel raw MAL messages
 * org.ccsds.moims.mo.mal.transport.tcpip.host ==> adapter (host / IP Address) that the transport will use for
 * incoming connections. In case of a pure client (i.e. not offering any services) this property should be omitted.
 * org.ccsds.moims.mo.mal.transport.tcpip.port ==> port that the transport listens to. In case this is a pure client,
 * this property should be omitted.
 *  
 * The general logic is the following : The transport at first initialises the server listen port (if this is a server, offering services).
 * 
 * On receiving a request to send a MAL Message the transport tries to find if it has allocated some recourses associated with the target URI (has already the means to exchange data with it) and if
 * not, it creates -numconnections- connections to the target server. If a client has already opened a connection to a server
 * the server will re-use that communication channel to send back data to the client. 
 * 
 * On the server, each incoming connection is handled separately by a different thread which on the first message reception associates the remote URI with the connection (socket). This has the
 * consequence that if the server wants to either use a service, or reply to the remote URI, it will use on of these already allocated communication resources.
 * 
 * In the case of malformed MAL messages or communication errors, all resources related to the remote URI are released and need to be reestablished.
 * 
 * URIs: 
 * 
 * The TCPIP Transpost, generates URIs, in the for of :
 * tcpip://<host>:<port or client ID>-<service id>
 * There are two categories of URIs Client URIs, which are in the form of
 * tcpip://<host>:<clientId>-<serviceId> , where the client id is a unique identifier for the client on its host, for example : 4783fbc147ab7aa56e7fff
 * and ServerURIs, which are in the form of
 * tcpip://<host>:<port>-<serviceId> and clients can actively connect to.
 * 
 * If a MAL instance does not offer any services then all of its endpoints get a Client URI.
 * If a MAL instance offers at least one service then all of its endpoints get a Server URI. 
 * A service provider communicates with a service consumer with the communication channel that the service
 * consumer initiated (uses bidirectional TCP/IP communication).   
 *  
...
public class TCPIPTransport implements MALTransport {
    ...
    /**
     * Number of processors that are capable of processing parallel input requests. This is the internal number of threads that process incoming messages arriving from MAL clients. It is the maximum
     * parallel requests this MAL instance can concurrently serve.
     */
    ...
    /**
     * The number of connections (sockets) per client or server. The Transport will connect numConnections times to the predefined port and host per different client/server.
     */
     ...
     * 
     * This method receives a raw MAL message, tries to decode it and then passes it to the higher layers of the MAL. If the message cannot be decoded, it will close the transport resource that
     * delivered this message and if this is a resource already associated with a URI, it will close all associations (sockets, resources) with the particular URI.
     * 
     * On receiving the first message from a transport resource, it will associate the remote URI with the resource in order to be able to send messages to this URI.
     * 
    ...
    /**
     * This method is the main entry point for sending a message to another MAL through the transport. It finds the appropriate transport resources associated to the URI to route this message to and
     * if they do not exist it creates them.
     * 
     * It then encodes and forwards the message to the appropriate transport resource and waits for a reply indicating if the transport was Successful in sending this message.
     * 
     * It is necessary for the caller thread to verify that the message was correctly sent otherwise there are no means to inform the higher leyers of the MAL that something went wrong.
     * 
	...
	try {
	    ...
	    // message is encoded!
	    LOGGER.log(Level.INFO, "TCPIP Sending data to {0} : {1}", new Object[] { remoteRootURI, packetToString(data) });
    ...
    /**
     * Creates a number of connections to a URI.
     * If the provided URI is a client URI (has a hex string in the place of the port , for example: 4783fbc147ab7aa56e7fff )
     * then there is no attempt to make a connection (it means that there are no active connections to the client)
     * 
    ...
    /**
     * This method returns a random Id to be used for differentiating different MAL instances
     * in the same host.
    ...
    /**
     * Used to inform the transport about communication problems with clients. In this case the transport will terminate all communication channels with the destination in order for them to be
     * re-established.
     * 
    ...
    /**
     * Returns the "root" URI from the full URI. The root URI only contains the protocol and the main destination and is something unique for all URIs of the same MAL.
     * 
