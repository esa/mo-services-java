...
/**
 * This class implements the low level data (MAL Message) transport protocol. The messages are
 * encoded according to the MAL TCPIP Transport Binding specification.
 *
    ...
    GENMessageSender
{
  ...
  /**
   * Read an encoded message from the socket. The message is read into a byte array. The encoded
   * message header contains the length of the variable-size body of the message. Therefore, this
   * information is already extracted. The resulting length is used to extract a body message from
   * the socket stream.
   *
   * If this TCPIP transport instance is also a provider, it has initiated a server socket with a
   * configuration-defined port. The incoming socket cannot have the same port, as it is not the
   * same socket. Hence, the port of the URI-to parameter in the message is modified such that it
   * equals the port of the server socket after the message is received. This ensures that the
   * communication supports several sockets, one for provider-side and one for client-side, while
   * still being compliant with the MAL restriction that every client/provider has exactly one
   * unique address.
   *
   * @return
   * @throws java.io.IOException
  ...
  public TCPIPPacketInfoHolder readEncodedMessage() throws IOException
  {
    ...
    try {
      ...
      RLOGGER.warning(
          "NullpointerException occured while reading header! " + headerReadNullPointer.getMessage());
    } catch (IndexOutOfBoundsException headerReadOutOfBounds) {
      RLOGGER.warning(
          "IndexOutOfBoundsException occured while reading header! " + headerReadOutOfBounds.getMessage());
    }
  ...
  private int readUntilComplete(final byte[] b, final int off, final int completeLength) throws
      IOException
  {
    int n;
    int len = 0;
