 ...
 */
public class TCPIPMessageBody implements MALMessageBody, java.io.Serializable {
    /**
     ...
     */
    public TCPIPMessageBody(final MALEncodingContext ctx, final Object[] messageParts) {
     ...
     *            parts.
     */
    public TCPIPMessageBody(final MALEncodingContext ctx, final boolean wrappedBodyParts, final MALElementStreamFactory encFactory, final MALElementInputStream encBodyElements) {
	this.ctx = ctx;
     ...
     */
    public void encodeMessageBody(final MALElementStreamFactory streamFactory, final MALElementOutputStream enc, final OutputStream lowLevelOutputStream, final UOctet stage, final MALEncodingContext ctx) throws MALException {
	...
	if ((null != messageParts) && (1 == messageParts.length) && (messageParts[0] instanceof MALEncodedBody)) {
	    ...
	    TCPIPTransport.LOGGER.log(Level.INFO, "GEN Message encoding body ... pc ({0})", count);
		...
		if (wrappedBodyParts) {
		    // we have more than one body part, therefore encode each
		    // part into a separate byte buffer, and then encode
		    // that byte buffer as a whole. This allows use to be able
		    // to return the complete body of the message as a
    ...
    private void encodeBodyPart(final MALElementStreamFactory streamFactory, final MALElementOutputStream enc, final boolean wrapBodyParts, final Object sf, final Object o, final MALEncodingContext ctx) throws MALException {
	...
	// else if it is a JAXB XML object
		...
		TCPIPTransport.LOGGER.log(Level.INFO, "GEN Message encoding XML body part : {0}", ow.toString());
	...
	if (!decodedBody) {
		...
		TCPIPTransport.LOGGER.log(Level.FINE, "GEN Message decoding body ... pc ({0})", bodyPartCount);
		...
		if (bodyPartCount == 1) {
		    ...
		    if (wrappedBodyParts) {
			TCPIPTransport.LOGGER.fine("GEN Message decoding body wrapper");
			...
			TCPIPTransport.LOGGER.log(Level.FINE, "GEN Message decoding body part : {0}", i);
			Object sf = null;
			if (!ctx.getHeader().getIsErrorMessage()) {
			    ctx.setBodyElementIndex(i);
		...
		TCPIPTransport.LOGGER.info("GEN Message decoded body");
	    } catch (MALException ex) {
		TCPIPTransport.LOGGER.log(Level.WARNING, "GEN Message body ERROR on decode : {0}", ex);
		ex.printStackTrace();
	    }
	}
    }
	...
	if (!(sf instanceof String)) {
	    ...
	    if (null != shortForm) {
		TCPIPTransport.LOGGER.log(Level.FINER, "GEN Message decoding body part : Type = {0}", shortForm);
		...
		TCPIPTransport.LOGGER.log(Level.FINER, "GEN Message decoding XML body part : Type = {0}", shortForm);
