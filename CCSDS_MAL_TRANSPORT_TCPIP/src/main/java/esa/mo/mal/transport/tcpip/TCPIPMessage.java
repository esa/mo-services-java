...
import esa.mo.mal.transport.tcpip.body.TCPIPDeregisterBody;
import esa.mo.mal.transport.tcpip.body.TCPIPErrorBody;
import esa.mo.mal.transport.tcpip.body.TCPIPMessageBody;
import esa.mo.mal.transport.tcpip.body.TCPIPNotifyBody;
import esa.mo.mal.transport.tcpip.body.TCPIPPublishBody;
import esa.mo.mal.transport.tcpip.body.TCPIPPublishRegisterBody;
import esa.mo.mal.transport.tcpip.body.TCPIPRegisterBody;
 ...
 */
public class TCPIPMessage implements MALMessage, java.io.Serializable {
    protected final TCPIPMessageHeader header;
    protected final TCPIPMessageBody body;
     ...
     *             is unknown.
     */
    public TCPIPMessage(final boolean wrapBodyParts, final TCPIPMessageHeader header, final Map qosProperties, final MALOperation operation, final Object... body) throws MALInteractionException {
	this.header = header;
	if (null == operation) {
     ...
     */
    public TCPIPMessage(final boolean wrapBodyParts, final boolean readHeader, final TCPIPMessageHeader header, final byte[] packet, final MALElementStreamFactory encFactory) throws MALException {
	...
	if (readHeader) {
	    ...
	    this.header = (TCPIPMessageHeader) enc.readElement(header, ctx);
	} else {
	    this.header = header;
	}
     ...
     * @throws MALException On decoding error.
     */
    public TCPIPMessage(final boolean wrapBodyParts, final boolean readHeader, final TCPIPMessageHeader header, final java.io.InputStream ios, final MALElementStreamFactory encFactory) throws MALException {
	this.qosProperties = new TreeMap();
	this.wrapBodyParts = wrapBodyParts;
     ...
     */
    public void encodeMessage(final MALElementStreamFactory streamFactory, final MALElementOutputStream enc, final OutputStream lowLevelOutputStream) throws MALException {
	try {
    ...
    private TCPIPMessageBody createMessageBody(final MALElementStreamFactory encFactory, final MALElementInputStream encBodyElements) {
	MALEncodingContext ctx = new MALEncodingContext(header, operation, 0, qosProperties, qosProperties);

	if (header.getIsErrorMessage()) {
	    return new TCPIPErrorBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	}
	...
	if (InteractionType._PUBSUB_INDEX == header.getInteractionType().getOrdinal()) {
		...
		return new TCPIPRegisterBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	    case MALPubSubOperation._PUBLISH_REGISTER_STAGE:
		return new TCPIPPublishRegisterBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	    case MALPubSubOperation._PUBLISH_STAGE:
		return new TCPIPPublishBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	    case MALPubSubOperation._NOTIFY_STAGE:
		return new TCPIPNotifyBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	    case MALPubSubOperation._DEREGISTER_STAGE:
		return new TCPIPDeregisterBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	    default:
		return new TCPIPMessageBody(ctx, wrapBodyParts, encFactory, encBodyElements);
	    }
	}
    ...
    private TCPIPMessageBody createMessageBody(final Object[] bodyElements) {
	MALEncodingContext ctx = new MALEncodingContext(header, operation, 0, qosProperties, qosProperties);

	if (header.getIsErrorMessage()) {
	    return new TCPIPErrorBody(ctx, bodyElements);
	}
	...
	if (InteractionType._PUBSUB_INDEX == header.getInteractionType().getOrdinal()) {
	    ...
	    switch (stage) {
	    case MALPubSubOperation._REGISTER_STAGE:
		return new TCPIPRegisterBody(ctx, bodyElements);
	    case MALPubSubOperation._PUBLISH_REGISTER_STAGE:
		return new TCPIPPublishRegisterBody(ctx, bodyElements);
	    case MALPubSubOperation._PUBLISH_STAGE:
		return new TCPIPPublishBody(ctx, bodyElements);
	    case MALPubSubOperation._NOTIFY_STAGE:
		return new TCPIPNotifyBody(ctx, bodyElements);
	    case MALPubSubOperation._DEREGISTER_STAGE:
		return new TCPIPDeregisterBody(ctx, bodyElements);
	    default:
		return new TCPIPMessageBody(ctx, bodyElements);
	    }
	}
