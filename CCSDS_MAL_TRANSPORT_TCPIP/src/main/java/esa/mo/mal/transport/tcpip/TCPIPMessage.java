...
/**
 * This TCPIP implementation of MAL Message provides encoding methods for encoding the MAL Message
 * according to the TCPIP Transport Binding red book.
 *
...
public class TCPIPMessage extends GENMessage
{
   ...
   *
   * Header and body are encoded separately, each with their own separate stream Factory. This is
   * done because the header needs to be encoded according to the specifications in the TCPIP
   * Transport Binding red book, but the body can be whatever.
   *
   * @param bodyStreamFactory    the stream factory to use for body encoding
   * @param lowLevelOutputStream the stream onto which both the encoded head and body will be
   *                             written
