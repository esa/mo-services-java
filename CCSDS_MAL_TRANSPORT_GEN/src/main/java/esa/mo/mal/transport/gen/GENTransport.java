...
public abstract class GENTransport<I, O> implements MALTransport
{
  ...
  /**
   * The delimiter to use to separate the external address part from the internal object part of the
   * URL.
   */
  protected final char serviceDelim;
  /**
   * If the protocol delimiter is the same as the service delimiter then we need a count to find the
   * correct service delimiter.
   */
  ...
  /**
   * The number of connections per client or server. The Transport will connect numConnections times
   * to the predefined port and host per different client/server.
   */
  private final int numConnections;
  /**
   * The thread that receives incoming message from the underlying transport. All incoming raw data
   * packets are processed by this thread.
   */
  private final ExecutorService asyncInputReceptionProcessor;
  /**
   * The thread pool of input message processors. All incoming messages are processed by this thread
   * pool after they have been decoded by the asyncInputReceptionProcessor thread.
   */
  ...
  /**
   * Map of outgoing channels. This associates a URI to a transport resource that is able to send
   * messages to this URI.
   */
  ...
  @Override
  public MALEndpoint createEndpoint(final String localName, final Map qosProperties) throws
      MALException
  {
  ...
  /**
   * On reception of an IO stream this method should be called. This is the main reception entry
   * point into the generic transport for stream based transports.
   *
   * @param receptionHandler The reception handler to pass them to.
   * @param decoder          The class responsible for decoding the message from the incoming
   *                         connection
   */
  public void receive(final GENReceptionHandler receptionHandler,
      final GENIncomingMessageDecoder decoder)
  {
    asyncInputReceptionProcessor.submit(new GENIncomingMessageReceiver(this, receptionHandler,
        decoder));
  }
      ...
      final GENMessage msg) throws MALTransmitErrorException
  {
    ...
    if (inProcessSupport
      ...
      receiveIncomingMessage(new GENIncomingMessageHolder(msg.getHeader().getTransactionId(), msg,
          new PacketToString(null)));
    } else {
      try {
        ...
        GENOutgoingMessageHolder outgoingPacket = internalEncodeMessage(remoteRootURI,
            destinationURI, multiSendHandle, lastForHandle, dataSender.getTargetURI(), msg);
        ...
        throw new MALTransmitErrorException(msg.getHeader(), new MALStandardError(
            MALHelper.INTERNAL_ERROR_NUMBER, null), null);
      } catch (Exception t) {
  ...
  /**
   * Used to request the transport close a connection with a client. In this case the transport will
   * terminate all communication channels with the destination in order for them to be
   * re-established.
   *
      ...
      synchronized (this) {
        ...
        if (commsChannel != null) {
          ...
          LOGGER.log(Level.WARNING,
              "Could not locate associated data channel to close communications, perhaps it was closed before. URI : {0} ",
              localUriTo);
        }
      }
      if (commsChannel != null) {
  ...
  /**
   * Used to inform the transport about communication problems with clients. In this case the
   * transport will terminate all communication channels with the destination in order for them to
   * be re-established.
   *
   * @param uriTo            the connection handler that received this message
   * @param receptionHandler
  ...
  /**
   * This method receives an incoming message and adds to to the correct queue based on its
   * transaction id.
   *
  ...
  /**
   * This method processes an incoming message by routing it to the appropriate endpoint, returning
    ...
    try {
      ...
      if (((type == InteractionType._SUBMIT_INDEX) && (stage == MALSubmitOperation._SUBMIT_STAGE))
        ...
        if ((null == ep) && (!endpointMalMap.isEmpty())) {
          ...
          LOGGER.log(Level.WARNING,
              "GEN Unable to return error number ({0}) as no endpoint supplied : {1}", new Object[]{
                errorNumber, oriMsg.getHeader()
              });
        }
      } else {
        LOGGER.log(Level.WARNING,
            "GEN Unable to return error number ({0}) as already a return message : {1}",
            new Object[]{
              errorNumber, oriMsg.getHeader()
            });
      }
    } catch (MALTransmitErrorException ex) {
      LOGGER.log(Level.WARNING, "GEN Error occurred when attempting to return previous error : {0}",
          ex);
    }
  }
  ...
  /**
   * Returns the "root" URI from the full URI. The root URI only contains the protocol and the main
   * destination and is something unique for all URIs of the same MAL.
   *
  ...
  /**
   * This method checks if there is a communication channel for sending a particular message and in
   * addition stores the communication channel on incoming messages in case of bi-directional
   * transports for re-use. If there is no communication channel for sending a message the transport
   * creates and registers it.
   *
   * @param msg                    The message received or to be sent
   * @param isIncomingMsgDirection the message direction
   * @param receptionHandler       the message reception handler, null if the message is an outgoing
   *                               message
   ...
   */
  protected synchronized GENConcurrentMessageSender manageCommunicationChannel(GENMessage msg,
      boolean isIncomingMsgDirection, GENReceptionHandler receptionHandler) throws
  ...
  /**
   * Registers a message sender for a given root URI. If this is the first data sender for the URI,
   * it also creates a GENConcurrentMessageSender to manage all the senders. If there are already
   * enough connections (numConnections) to the given URI the method does not register the sender.
   * This ensures that we will have at maximum numConnections to the target root URI.
   *
  ...
  /**
   * Method to be implemented by the transport in order to return a message sender capable if
   * sending messages to a target root URI.
   *
   ...
   */
  protected abstract GENMessageSender createMessageSender(GENMessage msg, String remoteRootURI)
      throws MALException, MALTransmitErrorException;

  /**
   * This Runnable task is responsible for decoding newly arrived MAL Messages and passing to the
   * transport executor.
   */
  private static class GENIncomingMessageReceiver implements Runnable
  {
    ...
    /**
     * This method processes an incoming message and then forwards it for routing to the appropriate
     * message queue. The processing consists of transforming the raw message to the appropriate
     * format and then registering if necessary the communication channel.
     */
    @Override
    public void run()
    {
  ...
  /**
   * This Runnable task is responsible for processing the already decoded message. It holds a queue
   * of messages split on transaction id so that messages with the same transaction id get processed
   * in reception order.
   *
   */
  private final class GENIncomingMessageProcessor implements Runnable
  {
    ...
    /**
     * Adds a message to the internal queue. If the thread associated with this executor has
     * finished it resets the flag and returns true to indicate that it should be resubmitted for
     * more processing to the Executor pool.
     *
