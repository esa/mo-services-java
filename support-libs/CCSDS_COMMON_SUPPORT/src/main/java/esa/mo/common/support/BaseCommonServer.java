...
public abstract class BaseCommonServer extends BaseComServer
{
   ...
   * Constructor.
   *
   * @param providerName The name of this provider to be used in the directory service.
   * @param domain the domain of the service provider.
   * @param network The network of the service provider.
   */
  public BaseCommonServer(final String providerName, final IdentifierList domain, final Identifier network)
  {
  ...
  /**
   * Helper method that creates the MAL provider instance and registers it in the Directory service.
   *
   * @param <T> The service provider type.
   * @param service The MAL service being provided.
   * @param supportCapabilities The supported capability numbers being provided.
   ...
   */
  protected <T extends MALInteractionHandler> T createAndPublishProvider(MALService service,
          Integer[] supportCapabilities,
          T handler,
          boolean isPublisher) throws MALException, MALInteractionException
  {
    ...
    directoryService.publishProvider(providerName, domain, network, service, scl, ep.getURI(), ep.getURI());
