...
/**
 * This class provides a set of helper methods for simplifying the generation of Activity Tracking events.
 */
public class ActivityTrackingPublisher
{
   ...
   * Constructor.
   *
   * @param eventProvider The event service provider used to publish activity tracking events.
   */
   ...
   */
  public ActivityTrackingPublisher(final EventServiceProvider eventProvider, final int nextInstanceIdentifier)
  {
    this.eventProvider = eventProvider;
  ...
  /**
   * Publishes an Acceptance event for a MAL interaction and constructs the source object from the interaction details.
   *
   ...
   */
  public void publishAcceptanceEventOperation(final MALInteraction interaction, final boolean success)
  ...
  /**
   * Publishes an Acceptance event for a MAL interaction using the supplied source object.
   *
   * @param interaction The MAL interaction to publish the event for.
   * @param success Success value for the activity tracking event.
   ...
   * @throws MALException On error.
   */
  public void publishAcceptanceEvent(final MALInteraction interaction, final boolean success, final ObjectId source)
          throws MALInteractionException, MALException
  {
    ...
    UpdateHeader uh = new UpdateHeader(StructureHelper.getTimestampMillis(), interaction.getMessageHeader().getURITo(), UpdateType.DELETION, ekey);
          ...
          throws MALInteractionException, MALException
  {
    ...
    {
      BaseMalServer.LOGGER.log(Level.WARNING, "ActivityTracking:raising SUBMIT execution event for interaction of type {0}",
  ...
  public void publishExecutionEventRequestResponse(final MALInteraction interaction, final boolean success) throws MALInteractionException, MALException
  {
    ...
    {
      BaseMalServer.LOGGER.log(Level.WARNING, "ActivityTracking:raising REQUEST execution event for interaction of type {0}",
              interaction.getMessageHeader().getInteractionType().toString());
    }
  ...
  public void publishExecutionEventInvokeAck(final MALInteraction interaction, final boolean success) throws MALInteractionException, MALException
  {
    ...
    {
      BaseMalServer.LOGGER.log(Level.WARNING, "ActivityTracking:raising INVOKE execution event for interaction of type {0}",
              interaction.getMessageHeader().getInteractionType().toString());
    }
  ...
  /**
   * Publishes an Execution event for a MAL interaction using the supplied stage count and index values.
   *
   * @param interaction The MAL interaction to publish the event for.
   * @param success Success value for the activity tracking event.
          ...
          final ObjectId source) throws MALInteractionException, MALException
  {
    ...
    UpdateHeader uh = new UpdateHeader(StructureHelper.getTimestampMillis(), eventSourceURI, UpdateType.DELETION, ekey);
