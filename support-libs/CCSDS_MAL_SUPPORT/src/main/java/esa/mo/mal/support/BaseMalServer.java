...
/**
 * A base class for service providers. Provides much of the boiler plate code needed for an MO service provider. Calling
 * methods, such as Main, should create an instance of a derived class, call init(), call start, and then when finished
 * call stop.
 */
public abstract class BaseMalServer
{
   ...
   *
   * @param localname Service provider local name, passed to the transport when creating an Endpoint, may be null.
   * @param protocol The transport protocol to use, may be null to use default.
   * @throws MALException If an error.
   * @throws MALInteractionException If an error.
   */
  public void init(final String localname, final String protocol) throws MALException, MALInteractionException
  {
    if (null == malFactory)
    {
    ...
    if (null == protocol)
    {
      transport = mal.getTransport(System.getProperty("org.ccsds.moims.mo.mal.transport.default.protocol"));
    }
    else
    {
   ...
   * Creates a MALConsumer to the supplied service and URIs.
   *
   * @param cep MALEndpoint to use, may be NULL in shich case the internal endpoint will be used.
   * @param service The service for the consumer.
   * @param uri The service and broker URI to use.
   * @return The MALConsumer instance.
   * @throws MALException On error.
   */
  public MALConsumer createConsumer(final MALEndpoint cep, final MALService service, final URI uri) throws MALException
  {
   ...
   * @throws MALException On error.
   */
  public MALConsumer createConsumer(final MALEndpoint cep, final MALService service, final URI uri, final URI brokerUri) throws MALException
  {
  ...
  /**
   * Sub classes must implement this and perform their MAL service type initialisation here.
   *
