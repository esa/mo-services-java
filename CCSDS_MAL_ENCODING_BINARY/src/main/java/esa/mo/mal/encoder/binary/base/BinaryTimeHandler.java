...
public class BinaryTimeHandler
{
  ...
  public void encodeTime(final BaseBinaryEncoder.BaseBinaryStreamHolder streamHolder, Time value) throws MALException
  {
    ...
    if (days > 65535)
    {
      ...
      throw new MALException(MessageFormat.format("Overflow of unsigned 16-bit days ({0}) when encoding MAL Time", days));
    }
  ...
  public void encodeFineTime(final BaseBinaryEncoder.BaseBinaryStreamHolder streamHolder, FineTime value) throws MALException
  {
    ...
    {
      // This check allows values bigger than maximum signed short, because the encoded value is an unsigned short
      throw new MALException(MessageFormat.format("Overflow of unsigned 16-bit days ({0}) when encoding MAL FineTime", days));
    }
    try
    {
      streamHolder.directAdd(java.nio.ByteBuffer.allocate(2).putShort((short) days).array());
      streamHolder.directAdd(java.nio.ByteBuffer.allocate(4).putInt((int) millisecondsInDay).array());
      streamHolder.directAdd(java.nio.ByteBuffer.allocate(4).putInt((int) picosecondsInMillisecond).array());
    }
    catch (IOException ex)
    {
      throw new MALException(IO_EXCEPTION_STR, ex);
  ...
  public Time decodeTime(final BaseBinaryDecoder.BaseBinaryBufferHolder inputBufferHolder) throws MALException
  {
    ...
    long millisecondsInDay = java.nio.ByteBuffer.wrap(inputBufferHolder.directGetBytes(4)).getInt() & 0xFFFFFFFFL;
  ...
  public FineTime decodeFineTime(final BaseBinaryDecoder.BaseBinaryBufferHolder inputBufferHolder) throws MALException
  {
    ...
    long days = java.nio.ByteBuffer.wrap(inputBufferHolder.directGetBytes(2)).getShort() & 0xFFFFL;
    long millisecondsInDay = java.nio.ByteBuffer.wrap(inputBufferHolder.directGetBytes(4)).getInt() & 0xFFFFFFFFL;
    long picosecondsInMillisecond = java.nio.ByteBuffer.wrap(inputBufferHolder.directGetBytes(4)).getInt() & 0xFFFFFFFFL;
