...
public class TestEncoder
{
  ...
  protected static void runtests(List<Results> results, int count, boolean testDecode, boolean dumpBuf) throws Exception
  {
    ...
    for (Results result : results)
    {
      if (result.compress)
      {
        check(result, new TestGZipStreamFactory(result.factory), count, testDecode, dumpBuf, result.objectToEncode, result.blankToEncode, ctx);
      }
      else
      {
        check(result, result.factory, count, testDecode, dumpBuf, result.objectToEncode, result.blankToEncode, ctx);
      }
    }
  ...
  protected static void check(Results result, MALElementStreamFactory streamFactory, int count, boolean testDecode, boolean dumpBuf, Object testComposite, Object blankComposite, MALEncodingContext ctx) throws Exception
  {
  ...
  protected static ByteArrayOutputStream testEncoder(Results result, MALElementStreamFactory streamFactory, int count, boolean dumpBuf, Object testComposite, MALEncodingContext ctx) throws Exception
  {
...
//      MALElementInputStream decoder = streamFactory.createInputStream(bbuf, 0);
  ...
  protected static class Results
  {
    ...
    public Results(String factoryClassName, boolean compress, boolean dump, int pktsPerReport, int paramsPerPkt, Time timestamp)
    {
      this.factoryClassName = factoryClassName;
      this.compress = compress;
      this.dump = dump;
