 ...
 * potentially encodes and delivers the MAL message via a HTTP response, or possibly processes the HTTP response as
...
public class HTTPMessageSenderRequestResponse extends HTTPMessageSenderNoResponse {
   ...
   *            the MALMessage
   * @throws IOException
   *             in case the message cannot be sent to the client
   */
  public void sendEncodedMessageViaHttpResponse(OutgoingMessageHolder<byte[]> packetData) throws IOException {
    ...
    try {
      ...
      RLOGGER.fine("sendEncodedMessageViaHttpResponse statusCode=" + statusCode);
  ...
  */
  protected void setResponseHeaders(MALMessageHeader malMessageHeader, IHttpResponse httpResponse)
      throws IOException {
  ...
  /**
   * If the default xml encoding mechanism is used, set the contenttype to application/mal-xml. Otherwise, set the
   * content-type to application/mal and specify the encoder used in the X-MAL-ENCODING header.
   * 
