...
public interface IHttpResponse {
   ...
   * 
   * @param statusCode
   *            the 3-digit integer code describing the status
   */
   ...
   *             in case an error occurs when writing the response body
   */
  public void writeFullResponseBody(byte[] data) throws HttpApiImplException;

  /**
   * Performs all required operations needed to commit the HTTP response, e.g. flush buffers, resume suspended
   * resources.
   * 
   * See notice at AbstractContextHandler's processResponse() method.
   * 
   * @throws HttpApiImplException
   *             in case an error occurs when committing the response
   */
