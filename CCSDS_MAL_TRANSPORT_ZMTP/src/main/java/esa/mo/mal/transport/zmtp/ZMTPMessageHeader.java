...
public class ZMTPMessageHeader extends GENMessageHeader
{
   ...
   * Constructor.
   *
   * @param configuration The ZMTP configuration to use for this message header
   * @param transport     Respective ZMTP transport that this message header is being handled by
   *
   */
  ...
  public Element decode(final MALDecoder hdrDecoder) throws MALException
  {
    ...
    if (version != ZMTP_BINDING_VERSION_NUMBER) {
      throw new MALException(MessageFormat.format(
          "Mismatching ZMTP version - supported: {0}, received: {1}", ZMTP_BINDING_VERSION_NUMBER,
          version));
    }
