 ...
 *
 * org.ccsds.moims.mo.mal.transport.tcpip.wrap org.ccsds.moims.mo.mal.transport.tcpip.debug == debug
 * mode , affects logging org.ccsds.moims.mo.mal.transport.tcpip.numconnections == number of
 * connections to a different MAL (either server / client)
 * org.ccsds.moims.mo.mal.transport.tcpip.inputprocessors == number of threads processing in
 * parallel raw MAL messages org.ccsds.moims.mo.mal.transport.tcpip.host == adapter (host / IP
 * Address) that the transport will use for incoming connections. In case of a pure client (i.e. not
 * offering any services) this property should be omitted.
 * org.ccsds.moims.mo.mal.transport.tcpip.port == port that the transport listens to. In case this
 * is a pure client, this property should be omitted.
 *
 * The general logic is the following : The transport at first initialises the server listen port
 * (if this is a server, offering services).
 *
 * On receiving a request to send a MAL Message the transport tries to find if it has allocated some
 * recourses associated with the target URI (has already the means to exchange data with it) and if
 * not, it creates -numconnections- connections to the target server. If a client has already opened
 * a connection to a server the server will re-use that communication channel to send back data to
 * the client.
 *
 * On the server, each incoming connection is handled separately by a different thread which on the
 * first message reception associates the remote URI with the connection (socket). This has the
 * consequence that if the server wants to either use a service, or reply to the remote URI, it will
 * use on of these already allocated communication resources.
 *
 * In the case of malformed MAL messages or communication errors, all resources related to the
 * remote URI are released and need to be reestablished.
 *
 * URIs:
 *
 * The ZMTP Transport, generates URIs, in the for of :
 * {@code malzmtp://<host>:<port or client ID>/<service id>} There are two categories of URIs Client
 * URIs, which are in the form of {@code malzmtp://<host>:<clientId>/<serviceId>} , where * the
 * client id is a unique identifier for the client on its host, for example : 4783fbc147ab7aa56e7fff
 * and ServerURIs, which are in the form of {@code malzmtp://<host>:<port>-<serviceId>} and clients
 * can actively connect to.
 *
 * If a MAL instance does not offer any services then all of its endpoints get a Client URI. If a
 * MAL instance offers at least one service then all of its endpoints get a Server URI. A service
 * provider communicates with a service consumer with the communication channel that the service
...
public class ZMTPTransport extends GENTransport<byte[], byte[]>
{
      ...
      final ZMTPURIMapping uriMapping) throws MALException
  {
    ...
    if (properties != null) {
      ...
      if (properties.containsKey(LOCAL_URI_PROPERTY_KEY)) {
        ...
        try {
          ...
          if (uri.getPort() == -1) {
            RLOGGER.log(Level.SEVERE, "Malformed URI (missing port): {0}", localURI);
          } else {
        ...
        RLOGGER.log(Level.INFO, "No local URI set. Generated a random URI: {0}", this.localURI);
      }
    } else {
      // default values - select random port
      this.localPort = getRandomSocketPort();
    ...
    RLOGGER.log(Level.INFO, "ZMTP Wrapping body parts set to : {0}", this.wrapBodyParts);
  }
  ...
  public void init() throws MALException
  {
    ...
    RLOGGER.log(Level.INFO, "Starting ZMTP PTP Channel Destination at {0} ({1})", new Object[]{
      localURI, mappedPtpURI
    });
    try {
      ptpDest = new ZMTPChannelDestination(this, ZMTP_COMMUNICATION_PATTERN_P2P, mappedPtpURI);
      ptpDest.runRxThread();
      RLOGGER.log(Level.INFO, "Started ZMTP PTP Channel Destination at {0} ({1})", new Object[]{
        localURI, mappedPtpURI
      });
    } catch (Exception ex) {
    ...
    if (mappedMcastURI != null) {
      RLOGGER.log(Level.INFO, "Starting ZMTP MCAST Channel Destination at {0} ({1})", new Object[]{
        localURI, mappedMcastURI
      });
      try {
        multicastDest = new ZMTPChannelDestination(this, ZMTP_COMMUNICATION_PATTERN_MULTICAST,
            mappedMcastURI);
        multicastDest.runRxThread();
        RLOGGER.log(Level.INFO, "Started ZMTP MCAST Channel Destination at {0} ({1})", new Object[]{
          localURI, mappedMcastURI
        });
      } catch (Exception ex) {
      ...
      RLOGGER.log(Level.INFO, "No ZMTP MCAST Channel Destination mapped from {0}", localURI);
    }
  ...
  public synchronized ZMTPChannelSource createMessageSender(String remoteRootURI) throws
      MALException
  {
    try {
      String mappedRemoteURI = uriMapping.getRemotePtpZmtpUri(remoteRootURI);
      ZMQ.Socket socket = openSocket(getZmqContext(), ZMTP_COMMUNICATION_PATTERN_P2P,
          mappedRemoteURI, false);
      ZMTPChannelSource channelSource = new ZMTPChannelSource(socket);
      return channelSource;
    } catch (IllegalArgumentException e) {
      RLOGGER.log(Level.WARNING, "Malformed parameters when creating sender to : {0}", remoteRootURI);
      throw new MALException("Malformed parameters when creating sender to : " + remoteRootURI);
    } catch (Exception e) {
      RLOGGER.log(Level.WARNING, "Exception" + e.toString() + " when creating sender to : {0}",
          remoteRootURI);
      throw new MALException(
          "Exception " + e.toString() + " when creating sender to : " + remoteRootURI);
    }
  }
  ...
  public GENMessage createMessage(byte[] packet) throws MALException
  {
    ...
    ZMTPMessageHeader header = new ZMTPMessageHeader(new ZMTPConfiguration(defaultConfiguration,
        qosProperties), null);
    ZMTPMessage dummyMessage = new ZMTPMessage(hdrStreamFactory, wrapBodyParts, true, header,
  ...
  {
    return new ZMTPEndpoint(this, defaultConfiguration, localName, routingName,
        uriBase + routingName, wrapBodyParts, properties);
  }
  ...
  /**
   * This method returns a random port number to be used for differentiating different MAL instances
   * in the same host.
   *
   ...
   * @param communicationPattern if P2P, ROUTER-DEALER sockets are created, if multicast, PUB-SUB
   *                             sockets are created
   ...
   */
  public static ZMQ.Socket openSocket(ZContext ctxt, int communicationPattern, String zmtpURI,
      boolean asServer) throws IllegalArgumentException
  {
    ZMQ.Socket ret = null;
    if (zmtpURI == null) {
