 ...
 *
 * Produces proper decoder for received messages and produces encoding bits for transmitted
 * messages.
 *
...
public class ZMTPEncodingSelector
{
    ...
    if (qosProperties.containsKey(MALZMTP_ENCODING_PROPERTY)) {
      ...
      if (outboundEncoder.equals(ENCODING_FIXED_BINARY_FACTORY)) {
        ...
        throw new MALException(MessageFormat.format(
            "Outbound encoder stream factory selected not recognized by ZMTPEncodingSelector: {0}",
            outboundEncoder));
      }
    } else {
  ...
  protected MALElementStreamFactory createEncodingFactory(String className, Map qosProperties)
      throws MALException
  {
    // Hack to force MALElementStreamFactory to produce desired factory
    System.setProperty(INTERNAL_ENCODING_PROPERTY, className);
    MALElementStreamFactory ret = MALElementStreamFactory.newFactory(INTERNAL_PROTOCOL_NAME,
        qosProperties);
   ...
   *
   * Selects a body decoder basing on encoding id and encoding extended id from given message header
   * @throws MALException
   */
  public MALElementStreamFactory getDecoderStreamFactory(ZMTPMessageHeader header) throws
      MALException
  {
    ...
    if (encodingId < ENCODING_OTHER_ID) {
      ...
      throw new MALException(MessageFormat.format(
          "Unsupported encoding selected - encodingId {0}, encodingExtId {1}", encodingId,
          encodingExtId));
    }
  }
