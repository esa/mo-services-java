...
/**
 * The MALProvider interface represents the execution context of a service provider for a given URI. If a service
 * provider is to be bound to several transport layers then several MALProviders should be created, each one
 * representing a binding between the service provider and a transport layer. NOTE – Those MALProviders should own the
 * same MALInteractionHandler instance. However, it is not mandatory, as several MALInteractionHandlers can share the
...
public interface MALProvider
{
   ...
   * @throws MALException If the MALProvider is not a publisher or if the MALProvider is closed.
   */
  ...
  /**
   * The method deactivates the MALInteractionHandler so that no interaction pattern is handled any longer.
   *
