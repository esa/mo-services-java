...
/**
 * The MALTransport interface enables the MAL layer to send and receive MAL messages through a single protocol. If
 * several protocols are used by the MAL layer then one MALTransport instance is created for each of them. Messages are
 * sent and received through communication ports represented by the interface MALEndpoint. A MALTransport is a factory
 * of MALEndpoint. One MALEndpoint is created for each MALConsumer and MALProvider. NOTE – If a MALProvider owns a
 * private broker, the MAL layer can create either two different MALEndpoints, one for the service provider and the
 * other for the broker, or a single one for both. In the first case, the service provider and its broker have two
 * different URIs whereas in the second case they share the same one.
 *
 */
public interface MALTransport
{
   ...
   * @param qosProperties QoS properties to be used when creating the MALEndpoint
   ...
   * @throws java.lang.IllegalArgumentException If the parameters ‘expectedQoS’ or ‘priorityLevelNumber’ are NULL
   * @throws MALException If the MALTransport is closed
   */
   ...
   * @param defaultQosProperties Default QoS properties used by the broker to send messages, may be null.
   * @return The new broker binding.
   * @throws java.lang.IllegalArgumentException If the parameters ‘endpoint’ or ‘expectedQoS’ or ‘priorityLevelNumber’
   * are NULL
   * @throws MALException If the MALTransport is closed
   */
  MALBrokerBinding createBroker(
